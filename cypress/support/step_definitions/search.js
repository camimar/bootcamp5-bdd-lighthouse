...
When(
  "User enters Film name as {string} and User clicks on search button",
  (movieName) => {
    Search.searchFor(movieName);
    Search.clickForSearch();
    cy.wrap(movieName).as("selectedMovie");
  }
);

Then(
  "User is able to successfully see results displayed for the search",
  () => {
...
When(
  "User enters Inexistent film name as {string} and User clicks on search button",
  (filmName) => {
    Search.searchFor(filmName);
    Search.clickForSearch();
  }
);

Then(
  "User is able to see validation message of inexistent films in Results section",
  () => {
    Search.elements
      .getNoResultsMessage()
      .should("be.visible")
...
When(
  "User starts to type syllables as {string} and User waits to see suggestions in box displayed under the search bar",
  (syllables) => {
    Search.searchFor(syllables);
    Search.elements.getSuggestionsBox().should("be.visible");
  }
);

Then(
  "User is able to successfully see suggestions displayed in the box",
  () => {
    Search.elements
      .getSuggestionItems()
